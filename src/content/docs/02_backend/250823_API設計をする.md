...
# API設計の具体的なプロセス: Todoアプリ用APIを立ててみて気づいたこと

## 記事概要

| 項目 | 内容 |
| ---- | ---- |
| 難易度 | ★★☆☆☆（API設計をしたことない人がAPI設計できるようになる）|
| 所要時間 | 約1時間（Todoアプリを具体化し、API設計書を書き切るまで with AI）|
| 検証環境 | なし |
| 目的 | API設計の具体的な思考プロセスやコツを整理する |
...
- **Todo アプリ用 API**を想定して、具体的なAPI設計プロセスを言語化。
...
- **API設計書**はMarkdown形式で記載する。

    理由:

  - Markdownファイルはテキストファイルであるため、**Gitで差分チェック・変更履歴管理**がしやすい
  - コードと同じリポジトリで一括管理できるため、**AIコーディングエージェントが参照・更新しやすい**（実装が先行して設計書の変更が追い付かないのを防げる）
...
「**どんな画面があって、各画面で何を表示させたいのか・どんな処理がなされるのか**」
...
- なぜAPI設計をする前に画面設計がある程度必要なのか？

    → 画面イメージがAPIを決める（APIは最終的には**画面で必要なデータをどう出すかだ**）から

#### 📝 補足

- 画面設計のコツ:

  - まずは**ホーム画面**（ログイン後トップ）から考える

    → ホーム画面で行う処理が大きい場合は、タブに分けたり、モーダルが出てくると良い

  - 「この画面で、ユーザーは何を見たい？何をしたい？」と問いかける
...
APIは**データへの操作**だから、
どんなデータ（モデル）があるか分からないとAPIが設計できない
...
**モデル（データ）を取得・作成・更新・削除するための入り口**。

→ そのため、**データが何か分からないと入り口は作れない**。

#### 📝 補足

画面から逆算して、最低限必要そうなモデルを決める（YAGNIを徹底）
...
各モデル（リソース）に対して、
**基本的なCRUD操作**（Create / Read / Update / Delete）を網羅的にAPI化する
...
- **リソース指向設計**の基本: 同じリソースに対する操作は、同じパスで**HTTPメソッドを変える**ことで表現
...
- **リソース指向の例外**として、「行為」をエンドポイントにする場合がある
...
実装前の最終チェックとして、
**フロントエンドから実際にAPIを呼び出す流れ**をシミュレーションし、
使いやすさ・効率性の観点から設計を見直す
...
- **GraphQLとの比較**
  ...
  - REST API: サーバー側が返すデータ構造を決める
  - GraphQL: クライアント側が必要なデータを指定できる

  → REST APIでも、クエリパラメータで柔軟性を持たせることで、GraphQLに近い使い勝手を実現可能
...
この記事が、API設計の具体的なプロセスを理解する助けになれば幸いです。
