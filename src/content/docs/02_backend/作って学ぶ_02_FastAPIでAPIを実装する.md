---
title: FastAPIによるAPI実装の仕方 ~Todoアプリ用APIを立ててみて気づいたこと~
...
**Pythonで高速なAPIを構築するためのWebフレームワークであるFastAPI**について、使い方の整理をしたいと思い本記事を作成しました。
...
- DB との接続はしていません。簡単のため、データはスクリプトに埋め込んでいます。今後分離を試みる予定です。
